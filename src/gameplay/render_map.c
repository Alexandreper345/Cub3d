...
/*   Updated: 2025/12/02 22:12:08 by alda-sil         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*static void render_line(t_vars *vars, int y_axis, char *line)
{
	...
	{
		if (i % 2 == 0)
			mlx_put_image_to_window(vars->mlx, vars->win, map->EA, x_axis,
				y_axis);
		else
			mlx_put_image_to_window(vars->mlx, vars->win, map->NO, x_axis,
				y_axis);
		x_axis = x_axis + 256;
		i++;
		printf("loppei 2\n");
