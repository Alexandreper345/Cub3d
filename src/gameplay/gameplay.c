...
/*   Updated: 2025/11/07 20:32:18 by erocha-l         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void draw_walls(t_map *map, t_camera *cam, t_player *player, t_vars *vars)
{    
    t_texture   *tex;

    tex = determinate_texture(cam, vars, map);
    if (cam->side == 0)
        cam->perpWallDist = (cam->mapX - player->posX + (1 - player->stepX) / 2) / cam->rayDirX;
    else
        cam->perpWallDist = (cam->mapY - player->posY + (1 - player->stepY) / 2) / cam->rayDirY;
